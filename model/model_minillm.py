#!/usr/bin/env python3
"""miniLLM 预训练使用的紧凑型 Decoder-only 大语言模型。
...
class MiniLLMConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # 当前紧凑版本暂未实现 KV Cache；生成时会重新计算整个上下文。
    ...
    def forward(
        ...
        with disable_autocast:
            ...
            if use_eager_attention:
                ...
                # Unpadded batches may use the selected SDPA implementation.
    ...
    def forward(
        ...
        if self.use_moe:
            (
                ...
                router_entropy_sum,
                routed_token_count,
            ) = self.mlp(hidden_states, token_mask=attention_mask)
        else:
    ...
    def forward(
        ...
        **_: Any,
        ...
        if sequence > self.config.max_position_embeddings:
            raise ValueError(
                f"sequence length {sequence} exceeds max_position_embeddings "
...
class MiniLLMForCausalLM(MiniLLMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # logits 形状为 [batch, sequence, vocab_size]。
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs: Any,
        ...
        # 当前紧凑版本没有 KV Cache，生成每个新 Token 时会重算完整前缀。
        return {
            "input_ids": input_ids,
            "attention_mask": attention_mask,
            "use_cache": False,
