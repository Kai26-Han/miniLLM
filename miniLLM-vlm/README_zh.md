...
miniLLM-vlm 是 [miniLLM](../README_zh.md) 的视觉语言扩展。项目复用已训练的 miniLLM Base 和 SigLIP 视觉编码器，通过一个轻量 Projector 对齐图像特征与语言嵌入，让原本只能处理文本的 miniLLM 具备单图理解、图像描述、视觉问答和多轮图文对话能力。
...
本项目的 Visual Pretrain 和 Visual SFT 均使用以下硬件完成，默认训练参数也围绕这一资源条件进行设计：

| 项目 | 配置 |
| --- | --- |
| GPU | 单卡 NVIDIA GeForce RTX 4090 |
| 训练方式 | 单机单卡 |
